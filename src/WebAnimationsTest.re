...
open Webapi.Dom; /* To avoid options needing to be unwrapped, just
 * create an element to use as a test: */

let element = Document.createElement("div", document); /* Smokes for Base interfaces: */
...
DocumentTimeline.setCurrentTime(
  sharedTimeline,
  500
...
Animation.reverse(myAnimation, ()); /* Promise-returning methods */

let readyPromise = Animation.ready(myAnimation);

let finishedPromise = Animation.finished(myAnimation); /* `animate` shortcut method */

let _ = element |> animate();

let _ =
  element |> animate(~effect=keyframeEffect, ~timeline=sharedTimeline, ());
